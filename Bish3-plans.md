...
* ~~把错误处理并入类型系统（就像*Rust*的`Result`）~~
  - ~~可以沿用`try`-`catch`的语法，但是去掉自动上传，改成`?`运算符显式上传~~
  - ~~这样也能自然地去掉那个诡异的`try`运算符~~
  - ~~还可以把可空类型的几个运算符推广到`Result`上~~
* 改革错误处理系统
  - 直接加上*Rust*的`Result`是不现实的，会引入新的染色问题，而且不利于重构（*Bish*在目前不太可能会有静态类型系统）
  - 但是，把可空类型的几个运算符推广到带异常的结果上确实很诱人
  - 然而，类似于现在的`try`运算符的设计并不是优雅的解决方案
* 加入`async`（没想好怎么设计）
  - 但无论如何我不太希望异步带有染色性
